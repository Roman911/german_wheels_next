	...
	return (
		<div className={ twMerge('gap-1.5 xl:gap-2.5 h-full', className) }>
			...
			<button
				...
				className={ twMerge('p-3 bg-[#E4E9F2] hover:bg-teal-300 rounded-full group', isBookmarks && 'bg-teal-300') }
			>
				<Icons.HeartIcon className='stroke-black group-hover:stroke-black w-4 h-4'/>
			</button>
			<button
				onClick={ () => handleClickComparison() }
				className={ twMerge('p-3 bg-[#E4E9F2] hover:bg-teal-300 rounded-full group', isComparison && 'bg-teal-300')
				}>
				<Icons.LibraIcon
					className='group-hover:fill-black fill-black w-4 h-4'/>
			</button>
		</div>
	)
};
