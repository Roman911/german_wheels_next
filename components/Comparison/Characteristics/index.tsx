...
export const Characteristics: FC<CharacteristicsProps> = ({ name, item  }) => {
	...
	return characteristics[name].map((i, index) => {
		if(i === 'season') {
			...
			return <Item key={ index } content={ item.best_offer.country ? 'ua' === Language.UA ? item.best_offer.country : item.best_offer.country_ru : '-' } />
		} else if(i === 'price') {
			return <div key={ index } className='h-11 leading-[44px] font-bold bg-[#E1E8F5]'>
				{ item.best_offer.price } ₴
			</div>
		} else if(i === 'load_index') {
			return <Item key={ index } content={ (item.load_index && item.load_index_ru) ? 'ua' === Language.UA ? item.load_index : item.load_index_ru : '-' } />
		} else if(i === 'speed_index') {
			return <Item key={ index } content={ (item.speed_index && item.speed_index_ru) ? 'ua' === Language.UA ? item.speed_index : item.speed_index_ru : '-' } />
